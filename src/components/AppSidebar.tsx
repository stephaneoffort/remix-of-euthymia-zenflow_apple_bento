...
export default function AppSidebar() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ type: "space" | "project"; id: string; name: string } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const newIds = hadKnown ? [...currentIds].filter((id) => !knownSpaceIds.has(id)) : [];
    if (newIds.length > 0) {
  ...
  const handleProjectDragEnd = useCallback(
    (event: DragEndEvent, spaceId: string) => {
      const { active, over } = event;
      if (!over || active.id === over.id) return;
  ...
  const handleAddProject = (spaceId: string) => {
    ...
    addProject(
      ...
      spaceId,
      newProjectColor,
      newProjectMemberIds.length > 0 ? newProjectMemberIds : undefined,
    );
    setNewProjectName("");
    ...
    if (type === "project") {
      ...
      if (project && project.spaceId !== spaceId) {
        ...
        toast({
          ...
          action: (
            <ToastAction altText="Annuler" onClick={() => moveProject(projectId, prevSpaceId)}>
              Annuler
            </ToastAction>
          ),
        });
      ...
      >
        ...
        {!isMobile && (
          <div
            ...
            onDoubleClick={() => {
              setSidebarWidth(256);
              localStorage.setItem("euthymia:sidebarWidth", "256");
            }}
        ...
        <div className="px-4 py-4 flex items-center justify-between border-b border-sidebar-border-color">
          ...
          <button
            ...
            className="p-1.5 rounded-md hover:bg-sidebar-hover text-sidebar-fg transition-colors btn-icon-touch flex items-center justify-center"
          >
          ...
          {addingSpace && (
            <div className="mb-2 mx-2 bg-sidebar-hover rounded-md p-2 space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  <span
                    className={`block w-3 h-3 rounded-full bg-background shadow absolute top-0.5 transition-transform ${newSpacePrivate ? "translate-x-4" : "translate-x-0.5"}`}
                  />
                </button>
              </div>
                ...
                <SortableSpace key={space.id} space={space}>
                      ...
                      >
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48">
                            <DropdownMenuItem
                              onClick={() =>
                                setAccessDialogSpace({ id: space.id, name: space.name, isPrivate: space.isPrivate })
                              }
                            >
                  ...
                  {expandedSpaces.has(space.id) && (
                    <div className="ml-4">
                      ...
                      >
                        <SortableContext
                          items={getProjectsForSpace(space.id).map((p) => p.id)}
                              ...
                              <SortableProject key={project.id} id={project.id}>
                                <ContextMenu>
                                  ...
                                  <ContextMenuContent className="w-44">
                                    <ContextMenuItem
                                      onClick={() => setMembersDialogProject({ id: project.id, name: project.name })}
                                    >
          ...
          <Collapsible defaultOpen={false}>
            <div className="px-4 py-2 border-t border-sidebar-border-color">
              <CollapsibleTrigger className="flex items-center justify-between w-full text-xs font-semibold text-sidebar-fg uppercase tracking-wider">
                <span className="flex items-center gap-1.5">
                  <Users className="w-3.5 h-3.5" /> Équipe & Paramètres
                </span>
                  ...
                  {deleteConfirm?.type === "space"
                    ? (() => {
                        ...
                        return (
                          <>
                            <p>
                              Supprimer l'espace <strong>« {deleteConfirm.name} »</strong> ?
                            </p>
                            <div className="rounded-md bg-destructive/10 border border-destructive/20 p-3 text-sm space-y-1">
                              ...
                              <ul className="list-disc list-inside text-muted-foreground">
                                <li>
                                  {spaceProjects.length} projet{spaceProjects.length > 1 ? "s" : ""}
                                </li>
                                <li>
                                  {spaceTasks.length} tâche{spaceTasks.length > 1 ? "s" : ""}
                                </li>
                              </ul>
                            </div>
                    ...
                    : (() => {
                        ...
                        return (
                          <>
                            <p>
                              Supprimer le projet <strong>« {deleteConfirm?.name} »</strong> ?
                            </p>
                            <div className="rounded-md bg-destructive/10 border border-destructive/20 p-3 text-sm space-y-1">
                              <p className="font-medium text-destructive">Éléments qui seront supprimés :</p>
                              <ul className="list-disc list-inside text-muted-foreground">
        ...
        {accessDialogSpace && (
          <SpaceAccessDialog
            ...
            onOpenChange={(open) => {
              if (!open) setAccessDialogSpace(null);
            }}
  ...
  return (
    <TooltipProvider>
      <Tooltip>
        ...
        <TooltipContent side="top">Installer l'app</TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
}
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <div
            {...attributes}
            {...listeners}
            className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1 opacity-0 group-hover/drag:opacity-60 cursor-grab active:cursor-grabbing z-10 p-0.5 hidden md:block"
          >
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <div
            {...attributes}
            {...listeners}
            className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2.5 opacity-0 group-hover/project:opacity-60 cursor-grab active:cursor-grabbing z-10 p-0.5 hidden md:block"
          >
            <GripVertical className="w-3 h-3 text-sidebar-fg" />
          </div>
        </TooltipTrigger>
        <TooltipContent side="left" className="text-xs">
          Glisser vers un espace pour déplacer
        </TooltipContent>
      </Tooltip>
      {children}
    </div>
...
/* ─── ESPACES (couleurs reprises de ta sidebar) ─── */
const SPACES = [
  { id: "euthymia", label: "Euthymia", color: "#7A4518", locked: false },
  { id: "administration", label: "Administration", color: "#C4A028", locked: false },
  { id: "ima", label: "IMA", color: "#2A7A5A", locked: false },
  { id: "formations", label: "Formations", color: "#2A4878", locked: false },
  {
    id: "communication",
    label: "Communication",
    color: "#2A5828",
    locked: false,
    children: [
      { id: "comm-digitale", label: "Communication Digitale", color: "#3A8048" },
      { id: "webinar", label: "Webinar", color: "#5A9060" },
      { id: "site-internet", label: "Site internet", color: "#7AB080" },
      { id: "mbpm", label: "MBPM com prog", color: "#8AB890" },
      { id: "retraite", label: "Retraite 5 avril 2026", color: "#9ACC9A" },
      { id: "personal", label: "Personal branding", color: "#AAD4AA" },
    ],
  },
  { id: "conception", label: "Conception formation", color: "#C45A28", locked: false },
  { id: "dharma", label: "Dharma", color: "#78286A", locked: true },
  { id: "2026", label: "2026", color: "#3A3028", locked: true },
  { id: "modeles", label: "Modèles", color: "#5A5040", locked: false },
];
...
export default function Sidebar({
  activeSpaceId = "communication",
  activeProjectId = "retraite",
  activeView = "Liste",
  activeNav = "dashboard",
  onNavChange = () => {},
  onSpaceChange = () => {},
  ...
  const activeProject = activeSpace?.children?.find((c) => c.id === activeProjectId);

  const navItem = (id: string, label: string, icon: React.ReactNode, badge?: number) => (
    <div
      key={id}
      ...
      style={{
        ...
        borderRadius: 8,
        cursor: "pointer",
        boxShadow: activeNav === id ? DN_SM : "none",
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        {icon}
        <span
          style={{ fontSize: 11, color: activeNav === id ? ACCENT : TEXT, fontWeight: activeNav === id ? 700 : 500 }}
        >
          {label}
        </span>
      </div>
  ...
  return (
    <div
      style={{
        ...
        flexShrink: 0,
        background: BG,
        borderRight: "1px solid rgba(140,118,88,0.15)",
        display: "flex",
    ...
    >
      {/* ── LOGO ── */}
      <div style={{ padding: "14px 14px 12px", borderBottom: "1px solid rgba(140,118,88,0.12)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
          ...
          >
            <div style={{ fontFamily: SERIF, fontSize: 15, fontWeight: 400, color: ACCENT }}>E</div>
          </div>
          <div>
            <div style={{ fontSize: 13, color: TEXT, fontWeight: 700 }}>Euthymia</div>
            <div style={{ fontSize: 9, color: MUTED }}>Gestion de projets</div>
          </div>
        </div>
      </div>

      {/* ── VOUS ÊTES ICI ── */}
      <div style={{ margin: "10px 10px 0", borderRadius: 12, boxShadow: DN, padding: "10px 12px" }}>
        <div
          style={{
            fontSize: 8,
            color: TEXT,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
          <div
            style={{
              ...
              height: 7,
              borderRadius: "50%",
              background: activeSpace?.color ?? ACCENT,
              flexShrink: 0,
            }}
          />
          <span style={{ fontSize: 10, color: FAINT, fontWeight: 500 }}>Espace</span>
          <span style={{ fontSize: 10, color: TEXT, fontWeight: 700, marginLeft: 2 }}>{activeSpace?.label ?? "—"}</span>
        </div>
        ...
        {/* Projet */}
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5, paddingLeft: 6 }}>
          <div
            style={{
              width: 6,
              height: 6,
              borderRadius: "50%",
              background: activeProject?.color ?? FAINT,
              flexShrink: 0,
            }}
          />
          <span style={{ fontSize: 10, color: FAINT, fontWeight: 500 }}>Projet</span>
          <span style={{ fontSize: 10, color: TEXT, fontWeight: 700, marginLeft: 2 }}>
            {activeProject?.label ?? "—"}
          </span>
        </div>

        {/* Flèche */}
        <div style={{ paddingLeft: 19, marginBottom: 5 }}>
          <svg width="8" height="10" viewBox="0 0 8 10" fill="none">
            <path
              d="M4 0v8M1.5 6L4 9l2.5-3"
              stroke={FAINT}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, paddingLeft: 12 }}>
          ...
          <span style={{ fontSize: 10, color: FAINT, fontWeight: 500 }}>Vue</span>
          <span
            style={{
              background: ACCENT,
              borderRadius: 100,
      ...
      <div style={{ padding: "10px 10px 4px" }}>
        ...
        {navItem(
          ...
          <svg width="12" height="12" viewBox="0 0 14 14" fill="none">
            <rect x="1" y="1" width="5" height="5" rx="1.5" fill={activeNav === "dashboard" ? ACCENT : TEXT} />
            <rect x="8" y="1" width="5" height="5" rx="1.5" fill={activeNav === "dashboard" ? ACCENT : TEXT} />
            <rect x="1" y="8" width="5" height="5" rx="1.5" fill={activeNav === "dashboard" ? ACCENT : TEXT} />
            <rect x="8" y="8" width="5" height="5" rx="1.5" fill={activeNav === "dashboard" ? ACCENT : TEXT} />
          </svg>,
        )}
        {navItem(
          "tasks",
          "Toutes les tâches",
          <svg width="12" height="12" viewBox="0 0 14 14" fill="none">
            <path d="M2 4h10M2 7h10M2 10h6" stroke={TEXT} strokeWidth="1.3" strokeLinecap="round" />
          </svg>,
        )}
        {navItem(
          "my-tasks",
          "Mes tâches",
          <svg width="12" height="12" viewBox="0 0 14 14" fill="none">
            <circle cx="7" cy="5" r="3" stroke={TEXT} strokeWidth="1.2" />
            <path d="M2 12c0-2.2 2.2-4 5-4s5 1.8 5 4" stroke={TEXT} strokeWidth="1.2" strokeLinecap="round" />
          </svg>,
        )}
        {navItem(
          "urgent",
        ...
        {navItem(
          ...
          "Aujourd'hui",
          <svg width="12" height="12" viewBox="0 0 14 14" fill="none">
            <rect x="2" y="2" width="10" height="10" rx="2" stroke={TEXT} strokeWidth="1.3" />
            <path d="M2 5h10M5 2v3M9 2v3" stroke={TEXT} strokeWidth="1.3" strokeLinecap="round" />
          </svg>,
        )}
        {navItem(
          "late",
          "En retard",
          <svg width="12" height="12" viewBox="0 0 14 14" fill="none">
            <circle cx="7" cy="7" r="5" stroke={TEXT} strokeWidth="1.3" />
            <path d="M7 4v3l2 2" stroke={TEXT} strokeWidth="1.3" strokeLinecap="round" />
          </svg>,
          overdueCount,
        )}
        {navItem(
          ...
          <svg width="12" height="12" viewBox="0 0 14 14" fill="none">
            <path
              d="M2 10.5C4 10.5 4 8 7 8s3 2.5 5 2.5M7 8V3M5 5l2-2 2 2"
              stroke={TEXT}
              strokeWidth="1.2"
              strokeLinecap="round"
              strokeLinejoin="round"
      ...
      {/* ── ESPACES ── */}
      <div style={{ padding: "8px 10px 4px", flex: 1, overflowY: "auto" }}>
        <div
          style={{
            display: "flex",
            alignItems: "center",
              ...
              <div key={space.id}>
                <div
                  onClick={() => {
                    ...
                    if (space.children) toggleSpace(space.id);
                  }}
                  style={{
                    display: "flex",
                    alignItems: "center",
                ...
                >
                  ...
                  <span
                    style={{
                      fontSize: 10,
                      color: isActive ? space.color : TEXT,
                  ...
                  {space.children && (
                    <span style={{ fontSize: 8, color: isActive ? space.color : FAINT }}>{isExpanded ? "▾" : "▸"}</span>
                  )}
                </div>

                {/* Sous-espaces */}
                {space.children && isExpanded && (
                  <div style={{ paddingLeft: 18, display: "flex", flexDirection: "column", gap: 1, margin: "1px 0" }}>
                    {space.children.map((child) => {
                      const isChildActive = child.id === activeProjectId;
                      return (
                        <div
                          key={child.id}
                          onClick={() => onProjectChange(child.id)}
                          style={{
                            display: "flex",
                            alignItems: "center",
                            gap: 5,
                            ...
                            borderRadius: 6,
                            cursor: "pointer",
                            boxShadow: isChildActive ? DN_XS : "none",
                          }}
                        >
                          <div
                            style={{ width: 5, height: 5, borderRadius: "50%", background: child.color, flexShrink: 0 }}
                          />
                          <span
                            style={{
                              fontSize: 9,
                              color: isChildActive ? child.color : TEXT,
        ...
        <div style={{ marginTop: 6 }}>
          ...
          >
            <svg width="11" height="11" viewBox="0 0 12 12" fill="none">
              <rect x="1" y="3" width="10" height="8" rx="1.5" stroke={TEXT} strokeWidth="1.1" />
              <path d="M1 5h10M4 7h4" stroke={TEXT} strokeWidth="1.1" strokeLinecap="round" />
              <path d="M3 3V2a1 1 0 011-1h4a1 1 0 011 1v1" stroke={TEXT} strokeWidth="1.1" />
            </svg>
            <span style={{ fontSize: 10, color: TEXT, fontWeight: 500, flex: 1 }}>Archives</span>
            <span
              style={{
                background: BG,
                borderRadius: 100,
      ...
      {/* ── BAS ── */}
      <div style={{ borderTop: "1px solid rgba(140,118,88,0.12)", padding: 10 }}>
        {/* Thème */}
        <div style={{ marginBottom: 8 }}>
          <div style={{ display: "flex", gap: 4, background: BG, borderRadius: 8, padding: 3, boxShadow: DN_SM }}>
            {(["light", "dark", "mixed"] as Theme[]).map((t) => (
              <button
                key={t}
                onClick={() => setTheme(t)}
                style={{
                  flex: 1,
                  background: theme === t ? BG : "transparent",
                  border: "none",
                  borderRadius: 6,
                  boxShadow: theme === t ? UP_XS : "none",
                  color: theme === t ? ACCENT : TEXT,
                  fontFamily: SANS,
                  fontSize: 9,
              ...
              >
                {t === "light" ? "☀ Clair" : t === "dark" ? "☽ Sombre" : "⊙ Mixte"}
              </button>
            ))}
          </div>
        </div>
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 8, color: MUTED }}>{userRole || "Membre"}</div>
          </div>
          <svg width="9" height="9" viewBox="0 0 10 10" fill="none">
            <circle cx="5" cy="5" r="4" stroke={TEXT} strokeWidth="1.1" />
            <path d="M3.5 4.5L5 6L6.5 4.5" stroke={TEXT} strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </div>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          ...
          >
            <svg width="13" height="13" viewBox="0 0 14 14" fill="none">
              <circle cx="7" cy="7" r="2" stroke={TEXT} strokeWidth="1.2" />
              <path
            ...
            <span style={{ fontSize: 8, color: TEXT, fontWeight: 500 }}>Réglages</span>
          </div>
          {/* Télécharger app */}
          <div
            style={{
          ...
          >
            <svg width="13" height="13" viewBox="0 0 14 14" fill="none">
              <rect x="2" y="2" width="10" height="10" rx="2" stroke={TEXT} strokeWidth="1.2" />
              <path
                d="M7 5v4M5 7l2 2 2-2"
                stroke={TEXT}
                strokeWidth="1.2"
                strokeLinecap="round"
                strokeLinejoin="round"
            ...
            <span style={{ fontSize: 8, color: TEXT, fontWeight: 500 }}>App</span>
          </div>
          {/* Déconnexion */}
          <div
            onClick={async () => {
          ...
          >
            <svg width="13" height="13" viewBox="0 0 14 14" fill="none">
              <path
                d="M9 2h2a1 1 0 011 1v8a1 1 0 01-1 1H9M6 10l4-3-4-3M10 7H2"
                stroke={DANGER}
                strokeWidth="1.2"
                strokeLinecap="round"
                strokeLinejoin="round"
            ...
            <span style={{ fontSize: 8, color: DANGER, fontWeight: 600 }}>Quitter</span>
          </div>
        </div>
      </div>
    </div>
