...
const QUICK_FILTERS: { key: QuickFilter; label: string; icon: React.ReactNode }[] = [
  ...
  // Native drag & drop: task dragged to a project
    ...
    if (type === 'task') {
      ...
      if (taskId) {
        ...
        if (targetLists.length > 0) {
          ...
          // We can't call hook here, so we need updateTask from the outer scope
        }
      }
    } else if (type === 'project') {
      const draggedProjId = e.dataTransfer.getData('projectId');
      const project = projects.find(p => p.id === draggedProjId);
      const targetSpace = spaces.find(s => getProjectsForSpace(s.id).some(p => p.id === projectId));
      if (project && targetSpace && project.spaceId !== targetSpace.id) {
        moveProject(draggedProjId, targetSpace.id);
      }
    }
  };
