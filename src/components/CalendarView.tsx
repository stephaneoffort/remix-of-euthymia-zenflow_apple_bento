      ...
      <div className="grid grid-cols-3 gap-1.5">
        {MONTHS_FR_SHORT.map((m, i) => (
          <button
            ...
            className={`px-2 py-1.5 rounded-md text-xs font-medium transition-colors ${i === month && pickerYear === year ? "bg-primary text-primary-foreground" : "hover:bg-muted text-foreground"}`}
          >
            {m}
          </button>
        ))}
  ...
  return (
    <HoverCardContent side="right" align="start" className="w-64 p-3 space-y-2 text-xs z-50">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span
          className={`px-1.5 py-0.5 rounded text-label font-medium ${PRIORITY_COLORS[task.priority] || PRIORITY_COLORS.normal}`}
        >
      ...
      {task.tags.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {task.tags.map((tag) => (
            <Badge key={tag} variant="secondary" className="text-label px-1.5 py-0">
              {tag}
            </Badge>
          ))}
        </div>
      ...
      {task.startDate && task.dueDate && task.startDate.slice(0, 10) !== task.dueDate.slice(0, 10) && (
        <p className="text-muted-foreground">
          {new Date(task.startDate).toLocaleDateString("fr-FR", { day: "numeric", month: "short" })} →{" "}
    ...
    <HoverCard openDelay={300} closeDelay={100}>
      <HoverCardTrigger asChild>
        ...
        >
          ...
          {isSpanning && spanInfo.isStart && !spanInfo.isEnd && (
          ...
          {task.googleEventId && meetTaskIds?.has(task.googleEventId) && (
            <img src={googleMeetIcon} alt="Meet" className="w-3 h-3 shrink-0" />
          )}
        </div>
      </HoverCardTrigger>
...
function DroppableDay({
  ...
  return (
    <div
      ref={setNodeRef}
      className={`group min-h-[100px] p-1.5 border-b border-r border-border transition-colors ${isCurrentMonth ? "bg-card" : "bg-muted/40"} ${isOver ? "bg-primary/10 ring-1 ring-inset ring-primary/30" : ""}`}
    >
      <div className="flex items-center justify-between">
        <span
          className={`text-xs font-semibold inline-flex items-center justify-center w-6 h-6 rounded-full ${isToday ? "bg-primary text-primary-foreground" : isCurrentMonth ? "text-foreground" : "text-muted-foreground"}`}
        >
          {dayNum}
        </span>
        {isCurrentMonth && (
...
function MobileTaskCard({
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left p-3 rounded-lg border transition-colors ${task.parentTaskId ? "ml-3 bg-accent/30 border-accent/50 hover:border-accent hover:bg-accent/40" : "bg-card border-border hover:border-primary/30 hover:bg-accent/30"}`}
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <span
          className={`shrink-0 px-1.5 py-0.5 rounded text-label font-medium ${PRIORITY_COLORS[task.priority] || PRIORITY_COLORS.normal}`}
        >
          {PRIORITY_LABELS[task.priority] || task.priority}
        </span>
      </div>
      <div className="flex items-center gap-2 mt-2">
        ...
        {task.googleEventId && meetTaskIds?.has(task.googleEventId) && (
          <img src={googleMeetIcon} alt="Meet" className="w-3.5 h-3.5 shrink-0" />
        )}
        {assignees.length > 0 && (
        ...
        {task.tags.length > 0 && (
          <div className="flex gap-1 overflow-hidden flex-1">
            {task.tags.slice(0, 2).map((tag) => (
              <Badge key={tag} variant="secondary" className="text-label px-1 py-0 shrink-0">
                {tag}
              </Badge>
            ))}
          </div>
...
function AgendaTaskList({
  ...
  setAddingForDate: (d: string | null) => void;
  newTaskTitle: string;
  setNewTaskTitle: (s: string) => void;
  handleAddTask: (d: string) => void;
  isMobile: boolean;
}) {
  return (
    <div className="space-y-2">
        ...
        dayTasks.map((t) =>
          isMobile ? (
            ...
            <button
              key={t.id}
              onClick={() => setSelectedTaskId(t.id)}
              className={`w-full text-left p-3 rounded-lg border transition-colors ${t.parentTaskId ? "ml-3 bg-accent/30 border-accent/50 hover:border-accent" : "bg-card border-border hover:border-primary/30"}`}
            >
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`shrink-0 px-1.5 py-0.5 rounded text-label font-medium ${PRIORITY_COLORS[t.priority] || PRIORITY_COLORS.normal}`}
                >
      ...
      {addingForDate === dateStr ? (
        <input
          ...
          onBlur={() => {
            if (newTaskTitle.trim()) handleAddTask(dateStr);
            else {
              setAddingForDate(null);
              setNewTaskTitle("");
            }
        ...
        <button
          ...
          className="w-full flex items-center gap-2 px-3 py-2 rounded-lg border border-dashed border-border text-muted-foreground hover:border-primary/30 hover:text-primary transition-colors text-sm"
        >
      ...
      {events.map((ev) => {
        ...
        const timeStr = ev.is_all_day
          ...
          : new Date(ev.start_time).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" }) +
            " – " +
            new Date(ev.end_time).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" });
        return (
          <div
            key={ev.id}
            className="group flex items-center gap-2 p-3 rounded-lg border border-border bg-muted/30 hover:border-primary/30 transition-colors cursor-pointer"
...
export default function CalendarView() {
  ...
  const handleAddTask = (dateStr: string) => {
    ...
    addTask({
      ...
      status: "todo",
      priority: "normal",
      dueDate: new Date(dateStr + "T00:00:00").toISOString(),
      startDate: null,
      assigneeIds: [],
      tags: [],
      parentTaskId: null,
        ...
        <Tooltip key={ev.id}>
          ...
          <TooltipContent side="right" className="text-xs max-w-[200px]">
            ...
            <p className="text-muted-foreground">
              ...
              {ev.last_synced_at &&
                ` · ${new Date(ev.last_synced_at).toLocaleDateString("fr-FR", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}`}
            </p>
      ...
      <>
        {visibleTasks.map((entry) => (
          <DraggableTask
            ...
            spanInfo={
              entry.totalDays > 1
                ? { isStart: entry.isStart, isEnd: entry.isEnd, totalDays: entry.totalDays }
                : undefined
            }
          ...
          <Popover>
            ...
            <PopoverContent className="w-72 max-h-64 overflow-y-auto p-2" side="bottom" align="start">
              <p className="text-xs font-semibold text-muted-foreground mb-2 px-1">
                {overflow} élément{overflow > 1 ? "s" : ""} supplémentaire{overflow > 1 ? "s" : ""}
              </p>
              <div className="flex flex-col gap-1">
                {all.slice(maxVisible).map((entry) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full shrink-0 ${STATUS_DOT_COLORS[t.status] || "bg-muted-foreground"}`}
                      />
                      <span className="text-xs text-foreground truncate flex-1">{t.title}</span>
                      <span
                        className={`shrink-0 px-1 py-0.5 rounded text-[10px] font-medium leading-none ${PRIORITY_COLORS[t.priority] || PRIORITY_COLORS.normal}`}
                      >
                        {PRIORITY_LABELS[t.priority] || t.priority}
                      </span>
                      {dueStr && <span className="shrink-0 text-[10px] text-muted-foreground">{dueStr}</span>}
                    </button>
                  );
                })}
    ...
    if (mode === "month") {
      ...
      const mCalDays = (() => {
        ...
        for (let i = 1; i <= lastDay.getDate(); i++)
          days.push({ date: new Date(mYear, mMonth, i), isCurrentMonth: true });
        const remaining = 42 - days.length;
        ...
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between px-3 py-2">
            ...
            <div className="flex items-center gap-1">
              ...
              <button onClick={() => navigateMobile(-1)} className="p-1.5 hover:bg-muted rounded-md transition-colors">
                <ChevronLeft className="w-4 h-4 text-foreground" />
              </button>
              <button
                onClick={() => setSelectedDay(new Date())}
                className="px-2 py-1 text-xs hover:bg-muted rounded-md transition-colors font-medium text-foreground"
              >
                Auj.
              </button>
              <button onClick={() => navigateMobile(1)} className="p-1.5 hover:bg-muted rounded-md transition-colors">
          ...
          <div className="grid grid-cols-7 gap-px bg-border mx-2 rounded-lg overflow-hidden">
            {DAYS_FR_SHORT.map((d, i) => (
              <div key={i} className="py-1 text-center text-label font-semibold text-foreground bg-muted/60">
                {d}
              </div>
            ))}
            {mCalDays.map((day, i) => {
              ...
              return (
                ...
                >
                  <span
                    className={`text-xs font-medium w-6 h-6 flex items-center justify-center rounded-full ${isTodayCell ? "bg-primary text-primary-foreground" : ""}`}
                  >
                    {day.date.getDate()}
                  </span>
                  {hasTasks && (
                    <span
                      className={`w-1 h-1 rounded-full mt-0.5 ${isSelected ? "bg-primary" : "bg-muted-foreground"}`}
                    />
                  )}
                </button>
              );
          ...
          <div className="px-3 py-1.5 border-t border-border bg-muted/30 mt-2">
            <p className="text-xs font-semibold text-foreground">
              {DAYS_FR_FULL[getFrDayIndex(selectedDay)]} {selectedDay.getDate()} {MONTHS_FR[selectedDay.getMonth()]}
            </p>
          </div>
          <div className="flex-1 overflow-y-auto px-3 py-2">
            ...
            <AgendaTaskList
              ...
              addingForDate={addingForDate}
              setAddingForDate={setAddingForDate}
            ...
            />
            <button
              onClick={() => handleOpenNewEvent(selectedDateStr)}
              className="w-full flex items-center gap-2 px-3 py-2 mt-1 rounded-lg border border-dashed border-border text-muted-foreground hover:border-primary/30 hover:text-primary transition-colors text-sm"
            >
        ...
        <div className="grid grid-cols-7 gap-1 px-2 pb-2">
          {mobileWeekDays.map((d, i) => {
            ...
            return (
              ...
              >
                <span
                  className={`text-label font-medium ${isSelected ? "text-primary-foreground" : "text-muted-foreground"}`}
                >
                  {DAYS_FR_SHORT[i]}
                </span>
                <span
                  className={`text-sm font-bold mt-0.5 ${isSelected ? "text-primary-foreground" : "text-foreground"}`}
                >
                  {d.getDate()}
                </span>
    ...
    <div className="space-y-3 mb-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button onClick={exportToICS} className="p-2 hover:bg-muted rounded-md transition-colors">
          ...
          <div className="flex gap-1">
            <button onClick={navigatePrev} className="p-2 hover:bg-muted rounded-md transition-colors">
              <ChevronLeft className="w-4 h-4 text-foreground" />
            </button>
            <button
              onClick={goToday}
              className="px-3 py-1.5 text-sm hover:bg-muted rounded-md transition-colors font-medium text-foreground"
            >
              Aujourd'hui
            </button>
            <button onClick={navigateNext} className="p-2 hover:bg-muted rounded-md transition-colors">
              <ChevronRight className="w-4 h-4 text-foreground" />
            </button>
          </div>
        </div>
      ...
      <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
        <div className="grid grid-cols-7 border border-border rounded-lg overflow-hidden flex-1">
          {DAYS_FR.map((d, i) => (
            <div
              key={i}
              className="py-2 text-center text-xs font-semibold text-foreground bg-muted/60 border-b border-border"
            >
              {d}
            </div>
          ))}
