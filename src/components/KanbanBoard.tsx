      ...
      >
        ...
        <div className="flex-1 space-y-2 overflow-y-auto scrollbar-thin">
          ...
          {newTaskStatus === status && (
            <div className="bg-card/95 backdrop-blur-sm rounded-lg border shadow-sm p-2.5 sm:p-3">
              <input
                autoFocus
                value={newTaskTitle}
        ...
        return (
          <div className="px-3 sm:px-6 pt-3 sm:pt-4 pb-1">
            <div className="flex items-center gap-2.5">
              ...
              <span data-numeric className="font-numeric text-label text-muted-foreground">{doneTasks}/{projectTasks.length}</span>
            </div>
          </div>
        );
      })()}
            ...
            {columnOrder.map(status => {
              ...
              return (
                ...
                >
                  ...
                  {getStatusLabel(status)}
                  <span data-numeric className={`font-numeric ml-0.5 tabular-nums ${isActive ? 'text-primary-foreground/80' : 'text-muted-foreground/60'}`}>{count}</span>
                </button>
              );
            })}
          </div>
