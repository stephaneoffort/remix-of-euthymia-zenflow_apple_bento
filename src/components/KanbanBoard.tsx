  ...
  const renderExpandedColumn = (status: string) => {
    ...
    return (
      <motion.div
        ...
        className={`flex flex-col w-[85vw] sm:w-0 sm:min-w-[10rem] sm:flex-1 shrink-0 sm:shrink rounded-lg snap-center sm:snap-align-none transition-all duration-200 ${
          draggedColumn === status ? 'opacity-40 scale-[0.97]' : ''
        } ${dropTargetColumn === status && draggedColumn ? 'ring-2 ring-primary/40 ring-offset-2 ring-offset-background' : ''}`}
        onDragOver={e => {
          e.preventDefault();
