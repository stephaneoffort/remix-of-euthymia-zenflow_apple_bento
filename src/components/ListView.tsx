            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between mt-1.5">
                  {task.dueDate ? (
                    <span className={`text-xs px-1.5 py-0.5 rounded dark:bg-muted ${isOverdue ? 'text-priority-urgent font-medium' : 'text-foreground'}`}>
    ...
    return (
      <React.Fragment key={task.id}>
        ...
        >
          ...
          <td className="py-2.5 px-3">
            {task.dueDate ? (
              <span className={`text-sm px-1.5 py-0.5 rounded dark:bg-muted ${isOverdue ? 'text-priority-urgent font-medium' : 'text-foreground'}`}>
                {new Date(task.dueDate).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}
              </span>
