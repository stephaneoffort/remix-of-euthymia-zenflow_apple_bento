...
import { RefreshCw, ChevronDown, ChevronUp, Search, ExternalLink } from 'lucide-react';
                ...
                return (
                  <div
                    ...
                    className={`rounded-lg border transition-colors cursor-pointer ${
                      msg.isUnread ? 'border-primary/30 bg-primary/5' : 'border-border bg-background'
                    }`}
                  >
                    <button onClick={() => setExpandedId(isExpanded ? null : msg.id)} className="w-full text-left p-2.5">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span className="text-[10px] text-muted-foreground shrink-0">{formatDate(msg.date)}</span>
                          </div>
                          <p className={`text-xs truncate ${msg.isUnread ? 'font-medium text-foreground' : 'text-foreground/70'}`}>
                            {msg.subject || '(sans objet)'}
                          </p>
                          {!isExpanded && (
                            <p className="text-[10px] text-muted-foreground truncate mt-0.5">{msg.snippet}</p>
                          )}
                        </div>
                        {msg.isUnread && <span className="w-2 h-2 rounded-full bg-primary shrink-0 mt-1.5" />}
                      </div>
                    </button>
                    ...
                    {isExpanded && (
                      <div className="px-2.5 pb-2.5 pt-0 border-t border-border/50">
                        ...
                        <p className="text-xs text-foreground/80 whitespace-pre-line leading-relaxed">{msg.snippet}</p>
                        <a
