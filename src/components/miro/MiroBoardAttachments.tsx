...
import { Plus, X, ExternalLink, Maximize2 } from 'lucide-react';
...
export default function MiroBoardAttachments({ entityType, entityId, defaultName }: Props) {
  const { isActive } = useIntegrations();
  const miro = useMiro();
  const [attachments, setAttachments] = useState<MiroAttachment[]>([]);
  const [loading, setLoading] = useState(true);
  const [pickerOpen, setPickerOpen] = useState(false);
  const [embedBoard, setEmbedBoard] = useState<MiroAttachment | null>(null);
  ...
  const handleDetach = async (att: MiroAttachment) => {
    try {
      await miro.detachBoard(att.id);
      setAttachments(prev => prev.filter(a => a.id !== att.id));
      toast.success('Tableau détaché');
    } catch {
    ...
    return (
      <div className="space-y-2">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
          <img src={INTEGRATION_CONFIG.miro.icon} alt="Miro" className="w-5 h-5 rounded" /> Miro
        </p>
        <Button variant="outline" size="sm" onClick={miro.connect} className="gap-1.5 text-xs">
          <img src={INTEGRATION_CONFIG.miro.icon} alt="" className="w-4 h-4 rounded" /> Connecter Miro
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        {miro.isConnected && (
          <Button variant="ghost" size="sm" onClick={() => setPickerOpen(true)} className="h-6 px-2 text-xs gap-1">
            <Plus className="w-3 h-3" /> Tableau
          </Button>
        )}
      </div>
      ...
      {loading ? (
        ...
        miro.isConnected ? (
          <p className="text-xs text-muted-foreground italic">Aucun tableau attaché</p>
        ) : null
      ) : (
        <div className="space-y-1.5">
          {attachments.map(att => (
            <div key={att.id} className="group flex items-center gap-2 p-1.5 rounded-md hover:bg-muted/50 transition-colors">
              {att.thumbnail_url ? (
                <img src={att.thumbnail_url} alt={att.board_name} className="w-8 h-8 rounded object-cover shrink-0" />
              ) : (
                <div className="w-8 h-8 rounded bg-[#FFD02F]/15 flex items-center justify-center shrink-0">
              ...
              <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button
                  onClick={() => window.open(att.board_url, '_blank')}
                  className="p-1 rounded hover:bg-muted"
                  title="Ouvrir dans Miro"
                >
      ...
      {embedBoard && (
        <MiroBoardEmbed
          boardId={embedBoard.board_id}
          boardName={embedBoard.board_name}
          open={!!embedBoard}
          onOpenChange={(v) => !v && setEmbedBoard(null)}
        />
      )}
    </div>
  );
